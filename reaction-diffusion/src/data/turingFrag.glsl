...
void main() {
    float dt = 0.000000000000000000000000000001;
    float dx = 0.00000000000000001;
    vec2 D = vec2(1.0, 100.0);

    float alpha = -0.005;
    float beta = 10.0;

    vec2 texPos = vec2(gl_FragCoord.x/sourceDimensions.x, gl_FragCoord.y/sourceDimensions.y);
    ...
    vec3 of = vec3(o + delta - texPos, 0.0);
